...
interface SidebarProps {  // this is the interface for the Sidebar component means the props that it will receive
    open: boolean
}

export default function Sidebar({ open }: SidebarProps) {
    const pathname = usePathname()  // this is the hook that we imported from next/navigation to get the current path
        ...
        <aside className={`bg-white border-r pt-20 border-gray-200 text-gray-800 w-64 fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
            <nav className="h-full flex flex-col justify-between">
                <div className="px-4 py-6 space-y-8">
                    {sidebarItems.map((item) => ( // this is the loop that will loop through the sidebarItems array and render the items and passHref is used to pass the href to the Link component
